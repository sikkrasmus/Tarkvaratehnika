<template>
  <v-app>
    ...
    <v-container grid-list-md>
      <v-flex 12>
        <v-card style="margin-top: 20px; background: linear-gradient(to bottom, #396afc, #2948ff);" color="blue-grey darken-2" class="white--text">
          ...
          <v-card-actions>
            <v-btn flat dark large v-on:click="addPortf">Add</v-btn>
          </v-card-actions>
        </v-card>
      </v-flex>
      <v-layout row wrap>
          <v-flex lg6 xs12 v-for="(portfolio, id) in this.portfolios" :key="value">
            <v-card style="margin-top: 20px; background: linear-gradient(to bottom, #00c6ff, #0072ff);" color="blue-grey darken-2" class="white--text">
              <v-card-title primary-title>
